...
# --- 10 ENLACES DE REFERIDOS Y PROTOCOLOS OFICIALES ---
...
SOLANA_TRACKER_URL = "https://partners.dub.co/solana-tracker"
HELIUS_DEV_URL = "https://www.helius.dev/?ref=angel-intel"
...
str_lit.markdown("<p style='font-size: 0.9rem; color: #94A3B8;'>Inspecciona de forma real cualquier Token, Wallet, Contrato o Firma en la blockchain de Solana. Obtén datos técnicos y la cartola de movimientos.</p>", unsafe_allow_html=True)
                ...
                if tx_data:
                    ...
                    str_lit.markdown(f"""
                        <div class='data-metric-box'>
                            ...
                            • <b>Estado en Red:</b> {status_text}<br>
                            • <b>Número de Bloque (Slot):</b> {slot}<br>
                            • <b>Fecha y Hora:</b> {time_str}<br>
                            • <b>Comisión Pagada (Fee):</b> {fee} SOL
                        </div>
                    """, unsafe_allow_html=True)
                else:
            ...
            try:
                ...
                if account_info:
                    ...
                    if isinstance(data_parsed, dict) and data_parsed.get("type") == "mint":
                        ...
                        mint_authority_status = f"REVOCADO. Detalle: {m_auth}" if m_auth is None else f"ACTIVO ({m_auth})"
                        freeze_authority_status = f"REVOCADO. Detalle: {f_auth}" if f_auth is None else f"ACTIVO ({f_auth})"
            except Exception:
                pass
            ...
            if pair_data:
                ...
                str_lit.markdown(f"""
                    <div class='data-metric-box'>
                        ...
                        • <b>Token:</b> {token_name} ({token_symbol})<br>
                        • <b>DEX:</b> {dex_name}<br>
                        • <b>Precio:</b> ${price_usd} USD<br>
                        • <b>Liquidez Total:</b> ${liquidity:,.2f} USD<br>
                        • <b>Volumen (24h):</b> ${volume_h24:,.2f} USD<br>
                        • <b>FDV:</b> ${fdv:,.2f} USD<br><br>
                        <b>🔒 ESTADO DE AUTORIDADES:</b><br>
                        • <b>Mint Authority:</b> {mint_authority_status}<br>
                        • <b>Freeze Authority:</b> {freeze_authority_status}
                    </div>
                """, unsafe_allow_html=True)
            else:
                str_lit.markdown(f"""
                    <div class='data-metric-box'>
                        ...
                        • <b>Dirección:</b> {query}<br>
                        • <b>Owner:</b> {account_owner}<br>
                        • <b>Balance:</b> {balance_sol:,.4f} SOL<br>
                        • <b>Mint Authority:</b> {mint_authority_status}<br>
                        • <b>Freeze Authority:</b> {freeze_authority_status}
                    </div>
                """, unsafe_allow_html=True)
                    ...
                    for idx, tx_item in enumerate(signatures_list, 1):
                        ...
                        str_lit.markdown(f"""
                            <div style='background: rgba(15, 23, 42, 0.6); border-left: 3px solid #38BDF8; padding: 10px; margin-bottom: 8px; border-radius: 4px; font-size: 0.85rem;'>
                                ...
                                <b>Fecha y Hora:</b> {time_str} (Slot: {slot})<br>
                                <b>TXID:</b> <code style='color:#FDE047;'>{sig}</code>
                            </div>
                        """, unsafe_allow_html=True)
                else:
                    str_lit.info("No se registran transacciones recientes públicas para este objetivo.")
            except Exception as e:
                str_lit.warning(f"No se pudo cargar la cartola: {e}")
        else:
            str_lit.error("Formato no reconocido. Ingrese una dirección válida de Solana (32-44 caracteres) o una firma.")
...
if str_lit.button("⚡ Actualizar Valores de Mercado"):
    str_lit.toast("¡Valores sincronizados!", icon="🚀")
...
def fetch_live_solana_market():
    ...
    for mint, name, sym in tokens:
        try:
            ...
            if pairs:
                p = pairs[0]
                results.append({"mint": mint, "symbol": sym, "name": name, "price": p.get("priceUsd", "0.00"), "change": p.get("priceChange", {}).get("h24", 0), "volume": p.get("volume", {}).get("h24", 0)})
            else:
...
# --- SECCIÓN 3: SERVICIOS PROFESIONALES Y SOC 24/7 ---
...
with col_s1:
    str_lit.markdown("""
        <div class='content-card'>
            ...
            <p style='font-size: 0.85rem; color: #94A3B8;'>Monitoreo continuo de red y respuesta a incidentes.</p>
            <div style='text-decoration: line-through; color: #94A3B8; font-size: 0.9rem;'>$1,500 USD</div>
            <div class='price-tag'>$1000 USD <span style='font-size:0.7rem; color:#38BDF8;'>/ mes</span></div>
        </div>
    """, unsafe_allow_html=True)
    if str_lit.button("Contratar SOC 24/7"):
        str_lit.success(f"Redirigiendo a pago. Tesorería: {TREASURY_WALLET_ADDRESS}")

with col_s2:
    str_lit.markdown("""
        <div class='content-card'>
            <h4 style='color: #F8FAFC; margin-top:0;'>🌐 Auditoría Web</h4>
            <p style='font-size: 0.85rem; color: #94A3B8;'>Pruebas de penetración OWASP Top 10.</p>
            <div style='text-decoration: line-through; color: #94A3B8; font-size: 0.9rem;'>$850 USD</div>
            <div class='price-tag'>$500 USD <span style='font-size:0.7rem; color:#38BDF8;'>/ único</span></div>
        </div>
    """, unsafe_allow_html=True)
    if str_lit.button("Contratar Auditoría Web"):
        str_lit.success("Iniciando solicitud de auditoría web.")

with col_s3:
    str_lit.markdown("""
        <div class='content-card'>
            <h4 style='color: #F8FAFC; margin-top:0;'>📱 Auditoría Android</h4>
            <p style='font-size: 0.85rem; color: #94A3B8;'>Ingeniería inversa y análisis de APK.</p>
            <div style='text-decoration: line-through; color: #94A3B8; font-size: 0.9rem;'>$850 USD</div>
            <div class='price-tag'>$500 USD <span style='font-size:0.7rem; color:#38BDF8;'>/ único</span></div>
        </div>
    """, unsafe_allow_html=True)
    if str_lit.button("Contratar Auditoría Android"):
        str_lit.success("Preparando entorno de análisis móvil.")
...
# --- SECCIÓN 5: PROTOCOLOS Y 10 REFERIDOS OFICIALES ---
str_lit.subheader("⚡ Protocolos y Red (10 Referidos Oficiales)")
str_lit.markdown("<p style='font-size: 0.9rem; color: #94A3B8;'>Acceso directo a herramientas de infraestructura, agregadores y bots integrados con tus enlaces de referido.</p>", unsafe_allow_html=True)
...
with col_r1:
    ...
    str_lit.markdown("<div class='content-card'><h4>🤖 Banana Gun Sniper Bot</h4></div>", unsafe_allow_html=True)
...
with col_r2:
    str_lit.markdown("<div class='content-card'><h4>⚡ Raydium Liquidity</h4></div>", unsafe_allow_html=True)
    ...
    str_lit.markdown("<div class='content-card'><h4>🐂 BullX Terminal</h4></div>", unsafe_allow_html=True)
    str_lit.link_button("Abrir BullX Terminal", BULKR_TRADING_URL)
    str_lit.markdown("<div class='content-card'><h4>⚡ Maestro Sniper Bot</h4></div>", unsafe_allow_html=True)
    str_lit.link_button("Abrir Maestro Bot", MAESTRO_BOT_URL)
    str_lit.markdown("<div class='content-card'><h4>🛠️ Helius Developer Portal</h4></div>", unsafe_allow_html=True)
